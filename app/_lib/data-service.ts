...
export const getCabins = async function (): Promise<GetCabinsPropsType[]> {
  const { data, error } = await supabase
    .from("cabins")
...
interface getCountriesPropTypes {
  name: string;
  flag: string;
  independent: boolean;
}
export async function getCountries(): Promise<getCountriesPropTypes[]> {
  try {
